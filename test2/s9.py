...
D = np.loadtxt('distante.txt')

# Datele problemei
n = len(D)  # numărul de insule

def calculeaza_distanta_totala(individ):
    depozite = individ[:-1].astype(int)
    distanta_totala = 0
    for i in range(n):
        distanta_minima = np.min([D[i][depozit] for depozit in depozite])
        distanta_totala += distanta_minima
    return distanta_totala

def fitness(individ):
    distanta_totala = calculeaza_distanta_totala(individ)
    return 1 / (1 + distanta_totala)  # Fitness mai mare pentru distanță totală mai mică

def gen_populatie(dim):
    pop = []
    for _ in range(dim):
        individ = np.random.choice(n, 3, replace=False)
